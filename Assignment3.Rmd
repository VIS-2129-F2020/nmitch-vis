...
### Calculation 2
The number and proportion of IndyGo Bus Stops located within 80 meters (approximately one block) of as Indiana Pacers Bikeshare dock.
...
```{r average distance bikes and bus}
...
ggplot(indy_neighborhoods) +
  ...
  geom_sf(data = indy_bus_stops, size = 0.1,
          aes(color = bike_dist)) +
  coord_sf(xlim = c(left_side, right_side), 
           ylim = c(bottom_side, top_side), expand = FALSE) +
  scale_color_viridis_c(name = 
                          "IndyGo Bus Stops\nby distance to a bike share location") +
  annotation_scale(location = "br") +
  annotation_north_arrow(location = "tr",
                         style = north_arrow_minimal()) +
  annotate(geom = "text", x = left_side + 300, 
           ...
           label = paste("On average, an IndyGo bus stop is\n ", 
                         ...
                         " meters from a Indiana Pacers Bike Share dock",
                         sep = ""),
...
```{r plot bike bus 2}
right_side <- st_bbox(indy_bike_share)$xmax
left_side  <- st_bbox(indy_bike_share)$xmin
top_side <- st_bbox(indy_bike_share)$ymax
bottom_side <- st_bbox(indy_bike_share)$ymin
...
### Calculation 6
The number and proportion of parks that overlap with cultural districts
...
```{r dist bus}
indy_dist <- indy_cultural_districts %>%
...
### Calculation 8
The number and proportion of IndyGo bus stops within 300 meters of a park. 

```{r, create parks buffer}
park_buffer <- st_buffer(indy_parks, dist = 300) %>%
  st_union()
```

```{r subsetting bus stops near park}
bus_stops_near_park <- indy_bus_stops[park_buffer,]
```
